...
begin
if messagebox(frmMain.Handle,'Вы уверены, что '+#10+#13
...
with frmMain do
  ...
  lbank.Color:=clGray;
  status.Caption:='Для начала игры нажмите кнопку "Раздать"';
  end;
frmMain.Show;
frmID.Hide;
end;
...
var newName: string;
begin
newName:=inputbox('Добавление нового игрока','Введи имя:','');
if (newName>'') and (length(newName)<=15) then
  ...
  igroki.Cells[3,igroki.RowCount-1]:='Стандартная';
  igroki.Row:=igroki.RowCount-1;
  end
  else
  showmessage('Имя не должно быть пустой строкой'
            +chr(10)+chr(13)+'или более 15 символов!');
end;
...
gamer: TGamer;
begin
igroki.Cells[0,0]:='Имя игрока';
igroki.Cells[1,0]:='Кол-во игр';
igroki.Cells[2,0]:='Балл';
igroki.Cells[3,0]:='Колода';
igroki.Cells[0,1]:='ИГРОК';
igroki.Cells[1,1]:='0';
igroki.Cells[2,1]:='0';
igroki.Cells[3,1]:='Стандартная';
...
if igroki.Cells[3,igroki.Row]='Эротическая' then
  cards.ItemIndex:=1
  else
  cards.ItemIndex:=0;
...
if igroki.Row>0 then
  if cards.ItemIndex=1 then
    igroki.Cells[3,igroki.Row]:='Эротическая';
  if cards.ItemIndex=0 then
    igroki.Cells[3,igroki.Row]:='Стандартная';
